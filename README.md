...
Для пагинации используются GET-параметры `_page`, `_per_page`. Параметр `_page` отвечает за то, какую страницу результатов отдавать (начиная с 0), а `_per_page` - сколько элементов должно быть на странице. Например, для запроса первой порции из 20 элементов будет использоваться следующий запрос.

```
GET /campaigns?_page=0&_per_page=20
```
