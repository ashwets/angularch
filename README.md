...
В prod-режиме должны логироваться только сообщения уровня error и warning. Это может быть осуществлено за счет конфигурирования logProvider.

Уровень логирования, по умолчанию, задается в `app/config.js`. Переключение на другой уровент на prod возможно с помощью куки `loggingLevel`.

Напр, в консоле можно выставить

```
document.cookie = "loggingLevel=debug";
```

и отлаживать приложение.
...
Есть множество движков для тестирование js-проектов. В целом, все они позволяют добиться примерно одного и того
же результата. Но, пожалуй, наилучший баланс между удобством и гибкостью дает [Jasmine](http://pivotal.github.io/jasmine/).

Запуск тестов происходит с помощью [Karma](http://karma-runner.github.io).

#### Тестирование сервисов ####

Все тесты располагаются в директории `test/spec`. Структура этой директории повторяет структуру самого проекта (директории `app/scripts`).
Например, для тестирования кода в `app/scripts/lib/filters.js` используется файл тестов `test/spec/lib/filters.js`.
...
Каждый тесткейс начинается с объявления `describe`, которое описывает, что тестируется в этом тесткейсе. Хорошей практикой
является указание в описании полного имени модуля и названия тестируемого сервиса. В примере тестируется сервис `Checkmark`
из модуля `lib.filters`.

Каждый тест начинается с объявления `it`, где кратко описывается, что проверяет тест. Сами проверки делаются с помощью `expect`
и матчеров, таких как `toEqual`. Полный список матчеров см. в документации по Jasmine.

Процедуры, котроые необходимо выполнить до каждого теста или после каждого теста, объявляются соответственно с помощью
`beforeEach` и `afterEach`. В примере используется два `beforeEach`. В первом следует указание, что необходимо загрузить
модуль `lib.filters`, во втором - происходит инийиализация фильтра `Checkmark`. Обратите внимание, что для использования
сервисов из контейнера Angular необходимо заворачивать функцию в вызов `inject`.


#### Тестирование контроллеров ####

Для тестирования большинства контроллеров необходимо использование моков, так как обычно контроллеры ожидают получения данных извне.
В большинстве случаев в моках надо описывать то, что должен вернуть сервер. Для этого используется сервиc `$httpBackend` и
методы `expect`.
...
describe('campaigns.controllers: CampaignListController', function () {
    ...
    it('should attach a list of 1 campaign', function () {
        ...
        mock.flush();
        expect(scope.campaignsTotal, 1);
        expect(scope.campaigns.length, 1);
        expect(scope.campaigns[0].name, "Elephant store");
        expect(scope.campaigns[0].startDate, Date(2013, 10, 10));
    });
});
```

Метод `expectGET` описывает url, который ответ которого нужно подменить. В описании url можно использовать регулярные выражения.
Метод `respond` описывет данные, которые должен возвращать подходящий url. `respond` принимает как структуру, так и функцию, с
 помощью которой можно делать интеллектуальные моки (см. `app/mock.js`).

После мокирования неоходимо инстанцировать контроллер через сервис `$controller`. Затем вызвать `flush`, который вернет данные
подписчикам контроллера.

Последовательность всегда должна быть именно такая:

* Описать моки.
* Вызвать контроллер.
* Сделать `flush`.

После этого можно проверять, что в итоге было помещено контроллером в `$scope`.

#### Запуск тестов ####

Есть два основных вида запуска тестов: dev-режим и тестирование сборки

##### Dev-режим #####

Первый способ используется при активной разработке и непосредственном написании тестов.

Есть опять же два варианта этого способа.
...
В этом случае поднимется сервер тестирования, тесты прогонятся один раз и сервер завершит работу. Это довольно удобный способ,
но может быть медленным при очень частом запуске тестов. Для ускорения можно разделить запуска на два этапа..
...
При этом сервер будет следить за изменениями и перезагружать код при необходимости.

В случае dev-режима необходимо с вниманием отнестись к поддержке файла конфигурции `karma.conf.js`.
В этой конфигурации необходимо перечислять все используемые файлы с кодом.

NB: по идее этот список файлов должен совпадать со списком из `index.html`. Поэтому есть вариант, что запуск karma можно
 совершать из grunt с предшествующим выдиранием конифига и `index.html`.

##### Тестирование сборки #####

Тестирование сборки предполагает запуск тестов на уже собранном приложении (после конкатенции, минификации и пр.).

Такой запуск осуществляется с помощью grunt.

```
grun test:dist
```

Этот запуск конфигурируется в `karma-dist.conf.js`.

Для запуска может использоваться параллельно несколько браузеров (напр., Chrome и Firefox).

#### E2e-тестирование ####

E2e тестирование должно производиться с использованием Selenium. Для этого можно использовать [Protractor](https://github.com/angular/protractor),
 но обычно Selenium-тесты поддерживаются отдельно, нет принципипальной необходимости писать их на js.
...
Также можно запустить собранное приложение (в том виде, в котором оно будет в продакшене).

```
grunt server:dist
```

В обоих случаях, чтобы не нажимать в браузере F5 при изменения рекомендуется поставить в браузер (LiveReload)[http://livereload.com/].
Плагины для разных браузеров можно найти здесь: http://feedback.livereload.com/knowledgebase/articles/86242-how-do-i-install-and-use-the-browser-extensions
