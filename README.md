...
Ссылки на роуты не должны просписываться явно типа `<a href="/campaigns/list/">list</a>`, а должны использовать
наименования маршуртов. Например: `<a ui-sref="campaigns.list'>list</a>`. Такой подход позволяет безболезненно менять
